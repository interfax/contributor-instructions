...
InterFAX provides a simple REST API for sending and receiving faxes. Faxes can be sent either by uploading a document (PDF, Word Doc, etc) to InterFAX as a "Document", or by providing the binary data in the body of a POST. Additionally a fax can be sent by pointing at any HTML page.
...
- Wrap the erroneous and fragile reading of binary dating and posting this to the server
- Provide with a simple DSL to make the API truly enjoyable to work with
...
To prevent abuse every developer account is limited to a single "inbound fax number". What this means is that you will only be able to send faxes to this number, which will also function as your own inbound number.
...
We are looking to cover all APIs available on the [InterFAX REST interface](https://www.interfax.net/en/dev/rest/reference). This includes all options and variations. In other words we want to cover:
...
We leave it to you the library builder to provide us with insight as to the possibility to create a custom DSL (domain specific language) for the InterFAX library in your language. For example, the Ruby library provides direct shortcuts to cancelling a fax from the fax object.
...
The source code should be published to github to the following location.
...
Please do not document the arguments and response in detail. Instead point to the REST reference documentation for the API call.
...
These samples should live in a seperate project. Ideally this should be named:
